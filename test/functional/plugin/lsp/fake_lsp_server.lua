...
function M.setup(test_name, timeout_ms, options)
  exec_lua([=[
    lsp = require('vim.lsp')
  ...
  ]=], test_name, M.code, M.logfile, timeout_ms or 1e3, options or {})
end

function M.test(config)
  if config.test_name then
    clear_notrace()
    M.setup(config.test_name, config.timeout_ms or 1e3, config.options)
  end
